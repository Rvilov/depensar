import "../../styles/sobreMi/SobreMiGrande.css";



export const SobreMiGrande = () =>{
    return(

        
       
            <div className="sobremi-grande-div">
                
                
                <div className="sobremi-grande-foto">
                   
                </div>
                               
                <div className="sobremi-grande-texto">
                    <h3>
                        Sobre Derlis
                    </h3>
                    <p>
                    Mi nombre es Derlis Lobo, fundadora de Depensar y licenciada en Psicología por la Universidad Yacambú, estado Lara, Venezuela. Complementé mi formación con un diplomado en Detección, Intervención y Tratamiento Psicológico. Mi pasión por la psicología clínica y de la salud me llevó a iniciar mi carrera en 2019, ayudando a personas a superar problemas emocionales con diversas técnicas psicológicas. 
                    <br></br>
                    <br></br>
                    En 2024, abrí mi propia consulta, enfocándome en asesoramiento y atención psicológica a adultos, adolescentes y niños. Mi objetivo es proporcionar un espacio seguro donde mis pacientes puedan explorar y fortalecer su bienestar emocional. 
                    <br></br>
                    <br></br>
                    A lo largo de mi carrera, he profundizado en temas de salud emocional, convencida de que es fundamental para mejorar la salud psicológica y prevenir futuros trastornos. El bienestar emocional nos permite desarrollar recursos internos para afrontar las dificultades y exigencias diarias.
                    <br></br>
                    <br></br>
                    En nuestras sesiones, trabajaremos juntos para reforzar herramientas que te ayuden a abordar tus preocupaciones, conocer mejor tus emociones, pensamientos y actitudes, y desarrollar habilidades para lograr un equilibrio emocional. Estoy comprometida a ofrecerte un espacio de apoyo y crecimiento para alcanzar un bienestar emocional pleno.
                    </p>
                   
                </div>
            </div>
        
    )}
